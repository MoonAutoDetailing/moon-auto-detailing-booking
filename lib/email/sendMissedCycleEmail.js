...
export async function sendMissedCycleEmailCore(cycleId) {
  ...
  if (!manageUrl) {
    ...
    console.warn("[SUBSCRIPTION_EMAIL] type=missed-cycle cycle_id=" + (cycle.id || cycleId) + " subscription_id=" + (cycle.subscription_id || "—") + " activation_booking_id_missing=" + activationMissing + " manage_token_missing=" + tokenMissing + " — Manage link omitted");
  }
