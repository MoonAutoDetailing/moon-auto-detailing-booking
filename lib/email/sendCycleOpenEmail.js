...
export async function sendCycleOpenEmailCore(cycleId) {
  ...
  if (!manageUrl) {
    const activationMissing = !activationBookingId;
    const tokenMissing = !!activationBookingId;
    console.warn("[SUBSCRIPTION_EMAIL] type=cycle-open cycle_id=" + (cycle.id || cycleId) + " subscription_id=" + (cycle.subscription_id || "—") + " activation_booking_id_missing=" + activationMissing + " manage_token_missing=" + tokenMissing + " — Manage link omitted");
  }
  ...
  const emailResult = await sendBookingEmail({
    ...
    html: `
      ...
      ${manageUrl ? `<p>Manage your subscription and book when ready:</p><p><a href="${manageUrl}">Manage Subscription</a></p>` : ""}
    `
  });
