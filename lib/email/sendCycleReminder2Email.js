...
/**
 * Send reminder 2 email (1 day before effective end). Uses activation booking manage_token for Manage link.
 */
export async function sendCycleReminder2EmailCore(cycleId) {
  ...
  const emailResult = await sendBookingEmail({
    to: customer.email,
    subject: "Moon Auto Detailing — Final reminder: book by tomorrow",
    html: `
      <h2>Your booking window closes soon</h2>
      <p>Hi ${(customer.full_name || "").split(" ")[0] || "there"},</p>
      <p>Your subscription booking window closes at the end of <b>${effectiveEnd || "—"}</b>. Book now to avoid missing this cycle.</p>
